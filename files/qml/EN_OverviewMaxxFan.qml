...
OverviewPage {
  ...
  title: qsTr("MaxxFan")
	...
	//Luftfeuchtigkeit
	VBusItem { id: ruuviHumidity; bind: Utils.path("com.victronenergy.temperature.ruuvi_cbad87dae0dc", "/Humidity")}
...
/*
	VBusItem { id: mqttIrSignal; bind: Utils.path("com.victronenergy.gps.mqtt_gps01_g1/MaxxFan/irSignal")}
	VBusItem { id: mqttAutoMode; bind: Utils.path("com.victronenergy.gps.mqtt_gps01_g1/MaxxFan/AutoMode")}
	VBusItem { id: mqttAirIn; bind: Utils.path("com.victronenergy.gps.mqtt_gps01_g1/MaxxFan/AirIn")}
	VBusItem { id: mqttAirOut; bind: Utils.path("com.victronenergy.gps.mqtt_gps01_g1/MaxxFan/AirOut")}
	VBusItem { id: mqttPower; bind: Utils.path("com.victronenergy.gps.mqtt_gps01_g1/MaxxFan/Power")}
	VBusItem { id: mqttOpen; bind: Utils.path("com.victronenergy.gps.mqtt_gps01_g1/MaxxFan/Open")}
	VBusItem { id: mqttTemperature; bind: Utils.path("com.victronenergy.gps.mqtt_gps01_g1/MaxxFan/Temperature")}
*/
	...
	// Temperaturauswahl
	//property int fanTemperature: 25
	//property int setTemperature: 25
	...
	// Power ON OFF
	//property int powerStatus: mqttPower.value

	// Lüfterstufe
	/*property int fanSpeed: maxxfan_status_fanspeed.value  // 0-10
	property int setFanSpeed: getFanspeed()
	...
	function getFanspeed(){
		if (maxxfan_status_fanspeed.value == 0) {
			...
			return maxxfan_status_fanspeed.value;
		}
	}
	*/
...
/*
 Image {
     source: "pics/maxx_fan_logo_200.png"
...
Button{
					...
					radius: 5
					baseColor: "#4C6981"
					pressedColor: "#3399ff" //: "#4C6981"
					//baseColor: "#3399ff"

						//opacity: mqttAutoMode.value  == 1 ? 1 : 0.4
						//visible: setAutoMode == 0
...
Rectangle {
			...
			TileText
						{
							//text: ruuviTempOutside.value.toFixed(1)
							//text: (ruuviTempInside.value - 10).toFixed(1)
...
// Animation for the rb_ventilator icon -> AIR-OUT (rechts)
    RotationAnimation {
        ...
        property: "rotation"
        //from: maxxfan_status_airflow.value == "out" ? 0 : 360
        //enabled: maxxfan_status_airflow.value == "out"
				from: 0
        to: 360
				duration: (4000 / maxxfan_set_fanspeed.value) * 2 // Duration in milliseconds for one full rotation
				//running: maxxfan_set_fanspeed.value > 0
				running: maxxfan_status_airflow.value == "out" && maxxfan_set_fanspeed.value > 0 && image_animation.value == 1
				loops: Animation.Infinite  // Infinite loop
    }
...
MouseArea {
	...
	/*onClicked: {
							...
							if (fanSpeed > 0)
								return maxxfan_ir_signal.setValue (maxxfan_status_airflow.text + fanSpeed)
							else if (fanSpeed == 0)
								return maxxfan_ir_signal.setValue ("close")
							}
*/
/*
onClicked: {
    maxxfan_set_fanspeed.value = maxxfan_status_fanspeed.value;
    if (maxxfan_status_fanspeed.value > 0) {
        maxxfan_ir_signal.setValue(maxxfan_status_airflow.text + maxxfan_status_fanspeed.value);
...
Button{
					...
					onClicked: {
						if (maxxfan_status_automode.value === 0){
							...
							//maxxfan_status_fanspeed.setValue (5)
							//maxxfan_ir_signal.setValue ("o5")
						}
...
Button{
					...
					height: 40
					radius: 5
					//baseColor: (maxxfan_status_airflow.value  == "i") && (mqttAutoMode.value == 0) ? "#3399ff" : "#4C6981"
...
Button{
					...
					radius: 5
					//enabled: false
					//baseColor: (maxxfan_status_airflow.value  == "o") || (mqttAutoMode.value == 1) ? "#3399ff" : "#4C6981"
					 ...
					 Rectangle {
					 	        ...
					 	        MbIcon {
					 	            ...
					 	            rotation: (maxxfan_status_airflow.value == "out") ? 0 : 180
					 							//x: arrowContainer.width / 2 - width / 3
					 						 ...
					 						 id: maxx_fan_icon_lid_bottom
 														...
 														RotationAnimation {
															         ...
															         //enabled: maxxfan_status_airflow.value == "out"
															 				from: 355
															         to: 345
															 				duration: 4000  // Duration in milliseconds for one full rotation
															 				//running: maxxfan_set_fanspeed.value > 0
														...
														RotationAnimation {
															         ...
															         //enabled: maxxfan_status_airflow.value == "out"
															 				from: 345
															         to: 355
															 				duration: 6000  // Duration in milliseconds for one full rotation
															 				//running: maxxfan_set_fanspeed.value > 0
															 ...
															 RotationAnimation {
																	 ...
																	 RotationAnimation {
																	         ...
																	         //enabled: maxxfan_status_airflow.value == "out"
																	 				from: 345
																	         to: 360
																	 				duration: 5000  // Duration in milliseconds for one full rotation
																	 				//running: maxxfan_set_fanspeed.value > 0
																	 				running: maxxfan_status_open.value == 0 && image_animation.value == 1
																	 				//loops: Animation.Infinite  // Infinite loop
																	     }
...
MbIcon {
					 ...
					 y: 21 //25
					 //opacity: 0.4
					 visible: image_animation.value == 1 //maxxfan_status_open.value == 0
					 width: 80 // 120
					 height: 25//21 // 54
					 anchors{
	 					horizontalCenter: fifthBox.horizontalCenter
...
//--------------------------------------------------------------- ANIMATED Images END
					 ...
					 Button{
					 					...
					 					height: 25
					 					radius: 5
										//baseColor: maxxfan_status_open.value  == 0 && maxxfan_status_fanspeed.value == 0 ? "#3399ff" : "#4C6981"
					 					...
					 					TileText {
					 						...
					 						font.pixelSize: 12
					 						font.bold: true
											//color: maxxfan_status_open.value  == 0 && maxxfan_status_fanspeed.value == 0 ? "#ffffff" : "#ffffff"
					 ...
					 Button{
					 					...
					 					//baseColor: maxxfan_status_open.value  == 1 || maxxfan_status_fanspeed.value > 0 ? "#3399ff" : "#4C6981"
										baseColor: maxxfan_status_open.value  == 1 || maxxfan_set_fanspeed.value > 0 ? "#3399ff" : "#4C6981"
					 					...
					 					TileText {
					 						...
					 						font.pixelSize: 12
					 						font.bold: true
											//color: maxxfan_status_open.value  == 1 || maxxfan_status_fanspeed.value > 0 ? "#ffffff" : "#ffffff"
